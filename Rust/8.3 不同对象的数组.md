...
否则 shapes 的类型会被自动推导为 `Vec<Box<Circle>>`，而第二个元素的类型却是 `Vec<Box<Square>>`，这将导致数组元素的类型不一致而报错。

```rust
    let shapes = vec![ // 自动推导为 Vec<Box<Circle>>
        Box::new(Circle { radius: 1.5 }),
        Box::new(Square { side: 2.0 }),   // ❌
    ];
```

如果声明 `shapes: Vec<Box<Draw>>` 也会报错，此处特征对象必须使用 dyn 关键字

#### Box 具有透明性

`for shape in shapes` 循环拿到的 shape 本质上是一个 `Box<dyn Draw>` 类型的对象，为什么 Box 对象可以直接调用 `draw()` 方法呢？

核心原因：[自动解引用](8.4%20自动解引用.md)  
这种机制让 Box 变成一个透明的容器，让你在使用 `Box` 时，感觉就像在操作它们内部的数据一样顺滑。
