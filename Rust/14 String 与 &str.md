...
> [!TIP]
> `&str` 既能表示字符串字面量，又能表示字符串切片。如果你要求一个变量必须是字符串字面量，你可以将其类型标注为 `&'static str`。

| **类型标注**              | **它的含义**             | **接受范围**                                    |
| --------------------- | -------------------- | ------------------------------------------- |
| **`s: &'static str`** | “我需要一个永远不会消失的字符串”    | **仅限**字符串字面量。                               |
| **`s: &str`**         | “我只需要看一眼这段字符串，不管它在哪” | **字面量**、**String 的切片**、**Vec<u8> 转的切片**。    |
| **`s: String`**       | “我需要拥有这段数据的所有权”      | **仅限** `String` 对象（或者通过 `.to_string()` 转换）。 |
